...
# MAGIC %md
# MAGIC # Gerar dados sintéticos – volume 5x (Motz Demo)
# MAGIC
# MAGIC Gera **cerca de 5x** a quantidade de dados do notebook `01_criar_tabelas_sinteticas` e grava nas mesmas tabelas Delta (overwrite).
# MAGIC
# MAGIC | Entidade        | 01 (base) | 05 (5x)  |
# MAGIC |-----------------|-----------|----------|
# MAGIC | Transportadoras | 50        | **250**  |
# MAGIC | Motoristas      | 200       | **1 000** |
# MAGIC | Embarcadores   | 40        | **200**  |
# MAGIC | Cargas realizadas | 150   | **750**  |
# MAGIC | Cargas disponíveis | 30   | **150**  |
# MAGIC | Cargas futuras  | 50        | **250**  |
# MAGIC | **Total cargas** | 230     | **1 150** |
...
# Configuração 5x – mesmo catalog/schema do projeto
CATALOG = "leticia_santos_classic_stable_catalog"
SCHEMA = "motz_demo"

# Volumes ~5x o notebook 01
NUM_TRANSPORTADORAS = 250
NUM_MOTORISTAS = 1000
NUM_EMBARCADORES = 200
NUM_CARGAS_REALIZADAS = 750
NUM_CARGAS_DISPONIVEIS = 150
NUM_CARGAS_FUTURAS = 250

SEED = 100  # seed diferente para variedade nos dados
...
for nome, df in [
    ...
    ("embarcadores", df_embarcadores),
    ("cargas", df_cargas),
]:
    print(f"  {nome}: {df.count()}")
